...
export class TeklifciEkleComponent  implements OnInit {
  ...
  teklifciEkle() {
    this.teklifciService.createFirmaYetkilisiAndFirma(this.firma,this.teklifci).subscribe(
      {
        next: (result)=> {this.messages=[
          { severity: 'success', summary: 'Teklifçi Eklendi', 
        ...
        error: (error)=>this.messages=[
          { severity: 'error', summary: 'Teklifçi Eklenenmedi', 
